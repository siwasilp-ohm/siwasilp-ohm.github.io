...
function handleImageUpload(event) {
    ...
    reader.onload = function(e) {
        ...
        originalImage.onload = function() {
            ...
            // Enable save original button
            document.getElementById('saveOriginalBtn').disabled = false;
            
            // Enable detection buttons
            document.getElementById('detectColorCheckerAuto').disabled = false;
            document.getElementById('detectColorCheckerManual').disabled = false;
...
let gridRotation = 0; // Rotation angle in degrees
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (saveOriginalBtn) {
        saveOriginalBtn.addEventListener('click', function() {
            saveImage(originalCanvas, 'original');
        });
    }
    
    const saveCorrectedBtn = document.getElementById('saveCorrectedBtn');
    if (saveCorrectedBtn) {
        saveCorrectedBtn.addEventListener('click', function() {
            saveImage(correctedCanvas, 'corrected');
        });
    }
});
...
function drawColorCheckerGrid() {
    ...
    // Calculate the center of the grid for rotation
    ...
    tempCtx.rotate(gridRotation * Math.PI / 180);
    tempCtx.translate(-centerX, -centerY);
    ...
    const rotationHandleY = gridStartY - rotationHandleDistance;
    
    // Draw line from center top to rotation handle
    tempCtx.beginPath();
    tempCtx.moveTo(centerX, gridStartY);
    tempCtx.lineTo(rotationHandleX, rotationHandleY);
    tempCtx.stroke();
    
    // Draw rotation handle
    tempCtx.fillStyle = 'rgba(0, 255, 0, 0.8)';
    tempCtx.beginPath();
    tempCtx.arc(rotationHandleX, rotationHandleY, handleSize / 2, 0, Math.PI * 2);
    tempCtx.fill();
    tempCtx.stroke();
...
function handleMouseDown(e) {
    ...
    // Check if we're on the rotation handle
    ...
    const dxRotation = mouseX - rotationHandleX;
    const dyRotation = mouseY - rotationHandleY;
    const distanceToRotationHandle = Math.sqrt(dxRotation * dxRotation + dyRotation * dyRotation);
    
    if (distanceToRotationHandle <= handleSize) {
        // We're on the rotation handle
        isRotating = true;
        
        // Calculate the initial angle between the center of the grid and the mouse position
    ...
    // Apply rotation transformation to corner coordinates
    const corners = [
        { x: gridStartX, y: gridStartY }, // top-left
        { x: gridStartX + gridWidth, y: gridStartY }, // top-right
        { x: gridStartX + gridWidth, y: gridStartY + gridHeight }, // bottom-right
    ...
    // Rotate corners based on current rotation
    const rotatedCorners = corners.map(corner => {
        ...
        // Rotate point
        const rotatedX = x * Math.cos(gridRotation * Math.PI / 180) - y * Math.sin(gridRotation * Math.PI / 180);
        const rotatedY = x * Math.sin(gridRotation * Math.PI / 180) + y * Math.cos(gridRotation * Math.PI / 180);
    ...
    for (let i = 0; i < rotatedCorners.length; i++) {
        const dx = mouseX - rotatedCorners[i].x;
        const dy = mouseY - rotatedCorners[i].y;
    ...
    // If not on a resize or rotation handle, check if we're inside the grid for dragging
    // For simplicity, we'll just check if we're inside the bounding box of the grid
    // A more accurate check would be to check if we're inside the rotated grid
...
function handleMouseMove(e) {
    ...
    if (isDragging) {
        ...
        // Calculate the current angle between the center of the grid and the mouse position
        ...
        gridRotation = (gridRotation + deltaAngle) % 360;
        
        // If rotation is negative, convert to positive angle
        if (gridRotation < 0) {
            gridRotation += 360;
        }
    ...
    // Redraw grid at new position/size/rotation
    drawColorCheckerGrid();
}
...
function confirmColorCheckerPosition() {
    ...
    // Store corners for distance calculation, applying rotation
    const corners = [
    ...
    colorCheckerCorners = corners.map(corner => {
        // Translate point to origin
        const x = corner.x - centerX;
        const y = corner.y - centerY;
...
function sampleColorsFromGrid() {
    ...
    for (let row = 0; row < 4; row++) {
        for (let col = 0; col < 6; col++) {
            // Calculate the center of the cell before rotation
            const cellCenterX = gridStartX + col * cellWidth + cellWidth / 2;
            const cellCenterY = gridStartY + row * cellHeight + cellHeight / 2;
            
            // Apply rotation to the cell center
            // Translate point to origin
            const x = cellCenterX - centerX;
            const y = cellCenterY - centerY;
            ...
            // Translate point back
            const sampleX = rotatedX + centerX;
            const sampleY = rotatedY + centerY;
            
            // Sample a 5x5 area and average the colors
            const sampleSize = 5;
            let r = 0, g = 0, b = 0;
            const imageData = originalCtx.getImageData(
                sampleX - Math.floor(sampleSize/2),
                sampleY - Math.floor(sampleSize/2),
                sampleSize,
                sampleSize
            );
            ...
            tempCtx.beginPath();
            tempCtx.arc(sampleX, sampleY, 3, 0, Math.PI * 2);
            tempCtx.fill();
...
function applyColorCorrection() {
    ...
    // Enable save buttons
    document.getElementById('saveOriginalBtn').disabled = false;
    document.getElementById('saveCorrectedBtn').disabled = false;
}
